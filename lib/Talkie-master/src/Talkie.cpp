...
#define PWM_INVERTED_VALUE_DESTINATION OCR2A
    ...
    // enable compare register B match interrupt to use TIMER1_COMPB_vect and not interfere with the Servo library
...
#define PWM_OUTPUT_FUNCTION(nextPwm) timer_set_compare(sTalkiePWMTimer, sTalkiePWMTimerChannel, nextPwm)
    ...
    // #define DAC_PIN PB3 // T2C4  - If the STM has a built in DAC, better use its pin.

#define PWM_OUTPUT_FUNCTION(nextPwm) analogWrite(PA5, nextPwm * 1.5f)
#define _10_BIT_OUTPUT
...
Talkie::Talkie(bool aUseNonInvertedOutputPin, bool aUseInvertedOutputPin)
{ // @suppress("Class members should be properly initialized")
...
static void setNextSynthesizerData()
{
    ...
    { // Energy = 15: stop frame. Silence the synthesizer and get new data.
        synthEnergy = 0;
        synthK1 = 0;
        synthK2 = 0;
        synthK3 = 0;
